...
const BenchmarkItem: React.FC<BenchmarkProps> = ({ 
  ...
  return (
    <div className={`glass-card p-8 animate-fade-in animate-delay-${delay}`}>
      <h4 className="text-xl font-semibold mb-6">{title}</h4>
      
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          <span className="text-white/70 text-sm">TableOne</span>
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center mb-2">
          <span className="text-white/70 text-sm">Industry Benchmark</span>
        ...
        <div className="w-full bg-white/10 rounded-full h-2.5">
          <div 
            className="bg-white/30 h-2.5 rounded-full"
            style={{ width: `${benchmarkValue}%`, transition: 'width 2s ease-in-out' }}
          ></div>
        </div>
      </div>
      
      <p className="text-white/60 text-sm mt-6">{description}</p>
    </div>
  );
};
